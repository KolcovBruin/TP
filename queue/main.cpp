...
void workWithClass(tmpl* queue) //шаблон класса  для работы c разными наследниками в основной функции, передаём указатель на класс
{
    ...
    do
    {
        ...
        case 4:
            if (!queue[number].check()) break;
                cout << "Число элементов, значение которых превышает значение предыдущего элемента: " << endl;
...
int main()
{
    ...
    if (guide==1)
    {
        ...
        cout << "Программа реализована с функцией выбора типа наследования"<< endl;
        cout << "Также для реализации копирования и слияния при запуски программы пользователю нужно выбрать кол-во необходимых ему очередей учитывая,что:"<< endl;
        cout << "-Для создания каждой новой копии необходима новая очередь" << endl;
        cout << "-Для создания каждого нового слияния необходима новая очередь" << endl;
        cout << "-Если не нужна функция копирования и слияния, то можно выбрать создание n-го  числа очередей только для копирования" << endl;
        cout << "-Если не нужна функция слияния, то можно выбрать создание n-го  числа очередей только для копирования" << endl;
    }
